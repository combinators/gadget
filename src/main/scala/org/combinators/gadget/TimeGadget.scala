...
import org.combinators.gadget.combinators.TemperatureFeature
...
class TimeGadget @Inject()(webJars: WebJarsUtil, applicationLifecycle: ApplicationLifecycle) extends InhabitationController(webJars, applicationLifecycle) with RoutingEntries   {
  ...
  val min30 = new PeriodicFeature(30, TimeUnit.Minute)
  val weatherFeature = new TemperatureFeature("Worcester", "01609", TemperatureUnit.Fahrenheit, min30)
  val extremeFeature = new ExtremaFeature(TimeUnit.Day)
  ...
  lazy val targets:Seq[Type] = Seq.empty :+ artifact(artifact.mainProgram, feature(FeatureClassification.Weather) :&: feature(FeatureClassification.Extrema) :&: feature(FeatureClassification.RepeatInInterval))
  ...
  lazy val controllerAddress: String = "org/combinators/gadget"
}
